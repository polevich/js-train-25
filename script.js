...
async function* asyncGenerator() {
  let i = 0;
  while (true) {
    await new Promise((resolve) => setTimeout(resolve, 1000)); // Затримка в одну секунду
    yield i++;
  }
}
...
async function printFiveItems() {
  for await (const value of gen) {
    console.log(value); // Виведення поточного значення
    if (value === 4) break; // Зупинка після виведення п'яти чисел
  }
}
...
async function displayData() {
  try {
    const gatherDataGenerator = gatherData();

    for (let i = 0; i < 3; i++) {
      const result = await gatherDataGenerator.next();
      console.log(result.value);
    }
  } catch (error) {
    console.error("Помилка під час виведення даних:", error);
  }
}
...
async function* gatherData() {
  try {
    ...
    throw new Error("Помилка під час збору даних: " + error.message);
  }
}
